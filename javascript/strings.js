// Lista de objetos representando chuvas de meteoros visíveis no hemisfério sul.
// Cada objeto possui:
// - nome: string -> nome da chuva de meteoros
// - pico: string -> abreviação do mês + dia (ex: "Fev 8")
// - velocidade: string -> velocidade com unidade (ex: "56 km/s")
...
// Exibe o título em letras maiúsculas
console.log(titulo.toUpperCase());
// Linha em branco para separar do conteúdo
console.log("\n");

// Itera sobre cada chuva de meteoros e formata os dados para exibição
for ( const chuva of chuvas_meteoros ) {

    // Extração de propriedades do objeto atual
    let nome_chuva = chuva.nome;
    let pico_chuva = chuva.pico;           // Ex: "Fev 8"
    let velocidade_chuva = chuva.velocidade; // Ex: "56 km/s"

    // Separa a string do pico em [mêsAbreviado, dia]
    let pico_mes_dia = pico_chuva.split(" ");
    
    // O formato esperado é sempre: abreviação do mês seguido do dia
    let pico_dia = pico_mes_dia[1]; 
    let pico_mes = pico_mes_dia[0]; 

    // Converte abreviação do mês para o nome completo em português
    let pico_mes_nome = "";
    ...
    switch (pico_mes) {
        ...
        // Em caso de abreviação inesperada, marca como inválido
        default:        pico_mes_nome = "Mês inválido";        break;
    }

    // Monta a data final legível: "8 de Fevereiro"
    let pico_data_chuva_formatada = pico_dia + " de " + pico_mes_nome;

    // Substitui a unidade curta por uma descrição mais legível
    let velocidade_formatada = velocidade_chuva.replace(" km/s", " quilômetros por segundo");

    // Impressão formatada com preenchimento de pontos para alinhamento
